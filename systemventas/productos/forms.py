...
from cris_forms.layout import Layout, Row , Column, Submit , Reset , ButtonHolder , Field , Div , HTML
from cris_forms.boostrap4 import AppendText , PrependedText , FormActions
...
class ProductoForm(forms.ModelForm): # formulario para el modelo Producto
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.helper.layout = Layout( # diseño del formulario con crispy forms
            Field('nombre'),
            Field('descripcion'),
            PrependedText('precio', '$', placeholder='0.00'), # agrega el simbolo de peso antes del campo precio
            Field('stock'),
            Field('stock_minimo'),
            Field('imagen'),
            ButtonHolder(
...
class MovimientoStockForm(forms.ModelForm): # formulario para el modelo MovimientoStock
    class Meta:
        model = MovimientoStock # modelo asociado al formulario
        fields = ['tipo', 'cantidad', 'motivo'] # campos del modelo a incluir en el formulario
        widgets = {
            'motivo': forms.Textarea(attrs={'rows': 3}), # widget para el campo motivo
        }
        labels = {
            'tipo': 'Tipo de Movimiento', # etiqueta personalizada para el campo tipo
            'cantidad': 'Cantidad',
        ...
        def clean_cantidad(self): # definimos el metodo clean para el campo cantidad
